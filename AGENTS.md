...
**Stack:** FastAPI (Python 3.11+, async) + Next.js (TypeScript, App Router) + Azure Cosmos DB
...
src/
 ...
 models.py     # Pydantic v2 models (DTOs)      
 db.py         # Data store operations      
...
tests/features/       # Behave BDD step definitions
...
|---|---|---|
| Unit tests (API) | `cd src/api && pytest` | pytest + httpx, async |
| Unit tests (API, watch) | `cd src/api && pytest --watch` | Re-runs on changes (pytest-watch) |
| Frontend lint | `cd src/web && npm run lint` | ESLint |
| Behave/Gherkin | `cd tests && behave` | BDD tests against Aspire |
...
| Playwright UI | `npx playwright test --ui` | Interactive debugging |
| All tests | `npm run test:all` | API + BDD + Playwright |

### Dev Server Commands

| Service | Command | URL |
|---|---|---|
| **Aspire (all services)** | `dotnet run --project apphost.cs` | API: http://localhost:5000, Web: http://localhost:3001, Docs: http://localhost:8000 |
| Backend (standalone) | `cd src/api && uvicorn app.main:app --reload --port 5000` | http://localhost:5000 |
| Frontend (standalone) | `cd src/web && npm run dev` | http://localhost:3000 |

> **Prefer Aspire** for all integration, Behave, and e2e testing. Standalone servers are only for isolated slice work.

### Build Commands

| Target | Command |
|---|---|
| Backend | `cd src/api && pip install -e .` |
| Frontend | `cd src/web && npm run build` |
| Frontend lint | `cd src/web && npm run lint` |
| API lint | `cd src/api && ruff check .` |

### Deploy Commands

| Command | Purpose |
|---|---|
| `azd provision` | Provision Azure resources (Container Apps, ACR, Cosmos DB, monitoring) |
