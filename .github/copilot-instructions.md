...
### Python / FastAPI Conventions ( src/api/)Backend 

- **Async endpoints**: All FastAPI route handlers must be `async def`. Use `await` for I/O operations
- **Pydantic v2 models**: Use Pydantic `BaseModel` for all request/response schemas. Use `model_validator` for complex validation
- **pydantic-settings**: Use `BaseSettings` for configuration. Environment variables are the source of truth
- **Type hints everywhere**: All function signatures must have type annotations. Use `Optional[]` for nullable fields
- **Snake_case**: snake_case for variables, functions, modules. PascalCase for classes/models only
- **Dependency injection**: Use FastAPI's `Depends()` for shared logic (auth, DB clients, config)
- **Error handling**: Raise `HTTPException` with appropriate status codes. Use custom exception handlers for structured error responses
- **Structured logging**: Use Python's `logging`  no `print()` in production codemodule 
- **Ruff for linting**: Use `ruff check .` and `ruff  configured in pyproject.tomlformat .` 
- **pytest for testing**: Use pytest with httpx `AsyncClient` for API tests. Use fixtures in `conftest.py`
...
- **App Router**: Use `page.tsx`, `layout.tsx`, `loading.tsx`, `error.tsx`  no `pages/` directoryconventions 
- **TypeScript strict mode**:  no `any` types, no implicit returns, explicit null checksEnabled 
- **Tailwind CSS**: Use utility classes for all  avoid custom CSS files or CSS-in-JSstyling 
...
- **Image optimization**: Use `next/ no raw `<img>` tagsimage` 
- **Link navigation**: Use `next/ no raw `<a>` tags for internal routeslink` 

### Test Framework Conventions

- **pytest**: Use `describe`-style grouping with classes or modules. Fixtures in `conftest.py`. Use `pytest-asyncio` for async tests
- **httpx**: Use `httpx.AsyncClient` with `app=app` for testing FastAPI without a running server
- **Behave**: BDD tests in `tests/features/`. Step definitions in `tests/features/steps/`. Tags: `@smoke`, `@api`, `@ui`
- **Playwright**: Use `test.describe()` for grouping. Page Object Model in `e2e/pages/`. No hardcoded waits
- **Test naming**: `test_should_[behavior]_when_[condition]` for pytest
